 ...
 */
package com.filesfm.android.ui.fragment;
...
import com.filesfm.android.files.FileMenuFilter;
import com.filesfm.android.lib.resources.status.OwnCloudVersion;
import com.filesfm.android.ui.activity.FileActivity;
import com.filesfm.android.ui.activity.FileDisplayActivity;
import com.filesfm.android.ui.activity.FileListOption;
import com.filesfm.android.ui.activity.FolderPickerActivity;
import com.filesfm.android.ui.activity.OnEnforceableRefreshListener;
import com.filesfm.android.ui.adapter.AdapterCallback;
import com.filesfm.android.ui.adapter.FileListListAdapter;
import com.filesfm.android.ui.dialog.ConfirmationDialogFragment;
import com.filesfm.android.ui.dialog.CreateFolderDialogFragment;
import com.filesfm.android.ui.dialog.RemoveFilesDialogFragment;
import com.filesfm.android.ui.dialog.RenameFileDialogFragment;
import com.filesfm.android.ui.helpers.SparseBooleanArrayParcelable;
import com.filesfm.android.ui.preview.PreviewAudioFragment;
import com.filesfm.android.ui.preview.PreviewImageFragment;
import com.filesfm.android.ui.preview.PreviewTextFragment;
import com.filesfm.android.ui.preview.PreviewVideoFragment;
 ...
 */
//Files.fm implements new Callback to get info from FileListListAdapter
public class OCFileListFragment extends ExtendedListFragment implements
    ...
    private void updateListOfFiles(
        ...
        mFileListAdapter = new FileListListAdapter(
                ...
                getActivity(),
                mContainerActivity,
                //Files.fm for Callback function
                this
        );
    ...
    //Files.fm complete new for adding functionality for each file and folder option button, functionality copied from
    //main menu functionality
    @Override
    public boolean onMethodCallback(OCFile file, MenuItem menuItem) {
        switch (menuItem.getItemId()) {
            ...
            case R.id.action_rename_file: {
                RenameFileDialogFragment dialog = RenameFileDialogFragment.newInstance(file);
                dialog.show(getFragmentManager(), FileDetailFragment.FTAG_RENAME_FILE);
                return true;
            }
            case R.id.action_see_details: {
            ...
            case R.id.action_send_file: {
                ...
                if (!file.isDown()) {  // Download the file
                    Timber.d("%s : File must be downloaded", file.getRemotePath());
                    ((FileDisplayActivity) mContainerActivity).startDownloadForSending(file);

                } else {
                    mContainerActivity.getFileOperationsHelper().sendDownloadedFile(file);
                }
                return true;
            }

            case R.id.action_remove_file: {
                RemoveFilesDialogFragment dialog = RemoveFilesDialogFragment.newInstance(file);
                dialog.show(getFragmentManager(), ConfirmationDialogFragment.FTAG_CONFIRMATION);
                return true;
            }
            case R.id.action_download_file:
            ...
            case R.id.action_set_available_offline: {
                mContainerActivity.getFileOperationsHelper().toggleAvailableOffline(file, true);
                getListView().invalidateViews();
                return true;
            }
            case R.id.action_unset_available_offline: {
                mContainerActivity.getFileOperationsHelper().toggleAvailableOffline(file, false);
                getListView().invalidateViews();
                invalidateActionMode();
                if (mFileListOption.isAvailableOffline()) {
                    onRefresh();
            ...
            //Files.fm complete new function opens file in browser using specific url for Files.fm server
            case R.id.action_open_in_browser: {
                FileDisplayActivity activity = (FileDisplayActivity) this.mContainerActivity;
                Account account = activity.getAccount();
                String name = account.name.substring(0 ,account.name.lastIndexOf("@"));
                Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("https://files.fm/server_scripts/filesfm_sync_contextmenu_action.php?username=" +
                        name + "&path="+ file.getRemotePath() + "&action=open_share"));
